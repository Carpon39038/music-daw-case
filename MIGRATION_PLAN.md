...
- [ ] 包含：Reverb(ConvolverNode)、Delay、Distortion(WaveShaper)、Compressor、Filter、StereoPanner
- [ ] App.tsx 中的播放逻辑迁移到 AudioEngine
- [ ] **不改动 UI**
- [ ] 验证：69 e2e 全通过

### Step 3: 组件拆分 ✅/⬜
- [ ] 从 App.tsx 拆出 Transport.tsx
- [ ] 从 App.tsx 拆出 TrackList.tsx
- [ ] 从 App.tsx 拆出 Timeline.tsx
- [ ] 从 App.tsx 拆出 Inspector.tsx
- [ ] 从 App.tsx 拆出 Mixer.tsx
- [ ] App.tsx 只做布局组合
- [ ] **保持现有布局不变，只拆文件**
- [ ] 验证：69 e2e 全通过

### Step 4: 布局重构 ✅/⬜
- [ ] 改为标准 DAW 布局：顶部 Transport + 左侧 TrackList + 右侧 Timeline + 底部 Mixer + 右侧 Inspector
- [ ] 全屏 h-screen，overflow hidden
- [ ] Clip 用彩色背景（track.color）+ 波形 SVG 预览
- [ ] 播放头三角形指示器
- [ ] 时间线 header 显示小节编号
- [ ] Loop 区域绿色半透明可视化
- [ ] 验证：69 e2e 全通过（可能需要更新部分 test selectors）

### Step 5: 交互增强 ✅/⬜
- [ ] 双击时间线创建 clip
- [ ] Clip 跨轨道拖拽
- [ ] Clip 右边缘拖拽调整 duration
- [ ] Shift+点击多选 clip
- [ ] 播放头可拖拽（点击时间线 header）
- [ ] 键盘快捷键：Space/S/Ctrl+Z/C/V/X/Delete
- [ ] 时间显示格式：MM:SS.ms (B{beat})
- [ ] 验证：69 e2e + 新增测试

### Step 6: 效果器 UI ✅/⬜
- [ ] Inspector 中 Track 效果器面板：Reverb/Delay/Distortion/Compressor/Filter
- [ ] 每个效果器可启用/禁用 + 参数滑块
- [ ] Track 声像(Pan)控制
- [ ] 验证：全测试通过
