          ...
          <details className="inspector-group sm" data-testid="inspector-clip" open>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div><label className="text-xs text-gray-500 block mb-1">Timbre (音色)</label><select data-testid="selected-clip-wave-select" value={selectedClipData.clip.wave} onChange={(e) => setSelectedClipWave(selectedClipData.track.id, selectedClipData.clip.id, e.target.value as WaveType)} disabled={isPlaying || selectedClipData.track.locked} className="w-full bg-[#1a1a1a] border border-gray-800 rounded px-2 py-1 text-sm focus:outline-none focus:border-emerald-500 text-gray-200"><option value="sine">柔和 (Sine)</option><option value="square">电子 (Square)</option><option value="sawtooth">锐利 (Sawtooth)</option><option value="triangle">温暖 (Triangle)</option><option value="organ">风琴 (Organ)</option><option value="brass">铜管 (Brass)</option></select></div>
              </div>
